				...
				<div class="row">
  				<div class="panel-body">
  					<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
						...
						<tbody>
						<c:choose>
							<c:when test="${not empty listlandSearch}">
								<c:forEach items="${listlandSearch}" var="land">
					...
					<nav class="text-center" aria-label="...">
					   ...
					   <c:choose>
								<c:when test="${page == 1}">
									 <li class="active"><a href="" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
								</c:when>
								<c:otherwise>
									<li class=""><a href="${pageContext.request.contextPath}/admin/land/search/?page=${page - 1}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
								</c:otherwise>
						</c:choose>
					      ...
					      <c:forEach begin="1" end="${numberOfPages}" step="1" varStatus="loop">
									<c:choose>
										<c:when test="${loop.count == page}">
											<li class="active"><a href="${pageContext.request.contextPath}/admin/land/search/?page=${loop.count}">${loop.count} <span class="sr-only">(current)</span></a></li>
										</c:when>
										<c:otherwise>
											<li class=""><a href="${pageContext.request.contextPath}/admin/land/search/?page=${loop.count}">${loop.count} <span class="sr-only">(current)</span></a></li>
										</c:otherwise>
									</c:choose>
							</c:forEach>
					      ...
					      <c:choose>
									...
									<c:otherwise>
										<li><a href="${pageContext.request.contextPath}/admin/land/search/?page=${page + 1}" aria-label="Next"><span aria-hidden="true">»</span></a></li>
									</c:otherwise>
							</c:choose>
